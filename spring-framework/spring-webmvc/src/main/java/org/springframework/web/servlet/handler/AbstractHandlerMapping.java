...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 */
	@Override  //day07：拦截器数据的初始化
	protected void initApplicationContext() throws BeansException {
		extendInterceptors(this.interceptors); //day07：拦截器怎么拿过来的
	 ...
	 */
	protected void detectMappedInterceptors(List<HandlerInterceptor> mappedInterceptors) { //day07：从容器中拿到所有的 MappedInterceptor
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		Object handler = getHandlerInternal(request); //day07：HandlerMapping的registry中找映射，返回HandlerMethod，真正执行当前请求的方法
		if (handler == null) {
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			if (interceptor instanceof MappedInterceptor) {
				...
				chain.addInterceptor(interceptor); //day07：所有拦截器加进去
			}
		}
		return chain;
	}
