...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			this.webApplicationContext = initWebApplicationContext(); //day02：初始化WebIOC容器
	...
	protected WebApplicationContext initWebApplicationContext() {
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext()); //day03：父容器
		WebApplicationContext wac = null; //day03：先会获取之前的 WebApplicationContext（构建父子容器）

		if (this.webApplicationContext != null) {
			// A context instance was injected at construction time -> use it
			wac = this.webApplicationContext; //day03：当前的web-ioc容器
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						cwac.setParent(rootContext); //day03：父子容器的体现，
					}
					configureAndRefreshWebApplicationContext(cwac); //day02：配置并且刷新容器
				}
			}
		}
		if (wac == null) {
	...
	protected void onRefresh(ApplicationContext context) {
		// For subclasses: do nothing by default. day06：留给子类的模板方法
	}
